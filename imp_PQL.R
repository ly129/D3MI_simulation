...
mice.impute.2l.bin.PQL <- function(y, ry, x, type,
  ...
  ##### glmmPQL syntax: y ~ x1 + x2, random = ~1 | id
  ...
  suppressWarnings(fit <- try(
    MASS::glmmPQL(fixed = formula(fixedpart),
                  random = formula(randompart),
                  family = binomial,
  ...
  if (!is.null(attr(fit, "class"))) {
    if (length(attr(fit, "class")) == 1) {
      if (attr(fit, "class") == "try-error") {
        warning("Binomial glmmPQL does not run. Simplify imputation model")
        return(y[wy])
      }
    }
  }
  ...
  psi.hat <- t(chol(fit$modelStruct$reStruct[[clust]])[1,1])^2
  
  s <- nrow(psi.hat) * psi.hat
  # rancoef <- as.matrix(lme4::ranef(fit)[[1L]])
  rancoef <- as.matrix(fit$coefficients$random[[clust]])
...
mice.impute.2l.cts.PQL <- function(y, ry, x, type,
                                   wy = NULL, intercept = TRUE, ...) {
  if (is.null(wy)) wy <- !ry
  ...
  if (inherits(fit, "try-error")) {
    warning("Gaussian glmmPQL does not run. Simplify imputation model")
    return(y[wy])
  }
  ...
  df <- fit$dims$N - length(fit$coefficients$fixed)
  ...
  covmat <- vcov(fit) * sigma2star / sigmahat^2
  rv <- t(chol(covmat))
  beta.star <- beta + rv %*% rnorm(ncol(rv))
  ...
  rancoef <- fit$coefficients$random[[1]]
  lambda <- t(rancoef) %*% rancoef
  df.psi <- nrow(rancoef)
...
poolres.binom <- function(fit,...){
  ...
  dimf <- length(fit$analyses[[1]]$coefficients$fixed)
  dimr <- 1 # length(as.data.frame(VarCorr(fit$analyses[[1]]))[,"vcov"])
  qfix <- matrix(NA, m, dimf)
  ufix <- matrix(0,  dimf, dimf)
  qran <- matrix(NA,  m, dimr)  
  for (i in 1:m){
    qfix[i,] <- fit$analyses[[i]]$coefficients$fixed
    ufix <- ufix + as.matrix(vcov(fit$analyses[[i]]))
    qran[i,] <- t(chol(fit$analyses[[i]]$modelStruct$reStruct[[1]]))^2 # as.data.frame(VarCorr(fit$analyses[[i]]))[,"vcov"]
  }
  ...
  df <- mice.df(m, lambda, fit$analyses[[1]]$dims$N-dimf-dimr-1, method = "smallsample")
...
poolres.cts <- function(fit,...){
  ...
  for (i in 1:m){
    qfix[i,] <- fit$analyses[[i]]$coefficients$fixed
    ufix <- ufix + as.matrix(vcov(fit$analyses[[i]]))
    qran[i,] <- c(t(chol(fit$analyses[[i]]$modelStruct$reStruct$st)) * fit$analyses[[i]]$sigma)^2 # as.data.frame(VarCorr(fit$analyses[[i]]))[,"vcov"][1:dimr]
  }
  Qbarfix <- apply(qfix, 2, mean)
  Ubarfix <- ufix/m
  Bfix <- cov(qfix)
...
mire.pql <- function(dat, fm,
  ...
  pred[pred[,"st"] != 0,"st"] <- -2
  imp <- mice(dat, print = F, pred=pred,
              method = method, maxit = MAX,
              m = M, verbose = FALSE)
  
  if (binom.out) {
    suppressWarnings(fit <- with(imp, glmmPQL(fixed = formula(fm),
                                              random = formula("~1|st"),
                                              family = binomial())))
  } else {
    suppressWarnings(fit <- with(imp, glmmPQL(fixed = formula(fm),
  ...
  # # models of x1 and x2
  # suppressWarnings(fitx1 <- with(imp, glmmPQL(fixed = x1 ~ x2 + x3,
  #                                             random = formula("~1|st"),
  #                                             family = binomial(),
  #                                             verbose = FALSE)))
  # resx1 <- poolres.binom(fitx1)
  # suppressWarnings(fitx2 <- with(imp, glmmPQL(fixed = x2 ~ x3,
  #                                             random = formula("~1|st"),
  #                                             family = gaussian(),
  #                                             verbose = FALSE)))
